            ...
            <div className="flex w-full flex-col sm:flex-row items-start sm:items-center justify-start sm:justify-between space-y-4 sm:space-y-0 mb-8">
                <SearchBox
                    ...
                    onSuggestionClick={(suggestion) => {
                        ...
                        handleSearchQueryChange(syntheticEvent);
                    }}
                />
...
const ProductsGrid = (props: ProductsGridProps) => {
    ...
    return (
        <div className={`${props.viewMode === 'list'
            ? 'space-y-4'
            : `grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 ${props.isAdmin ? "py-8 border-gray-500 border-t border-b" : ""}`
        }`}>
            {productsComponent}
        </div>
    );
